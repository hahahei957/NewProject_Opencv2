...
def step_gradient(b_current, w_current, points, lr):
    ...
    # 这里通过这些之所以可以实现梯度下降，是因为特征值是一个一维的，所以没有类似二维的方向导数，导数只有一个，每次下降的方向只有一个，求出来的就是每次下降方向上的导数
    #  这里求出来的b_gradient和w_gradient是误差函数在每个样本点处计算出来的误差的导数的平均，
    #  而导数乘以横坐标就是纵坐标(∆y=∆x*tan𝞱  而tan𝞱就是导数值)，随着误差值一点一点降低，导数值将不断变小，从而实现梯度下降
    #  所以这里的学习率表示每一次前几多少，也就相当于x方向的微小增量∆x，而梯度下降的距离大小也就是∆y等于lr * b_gradient
...
def gradient_descent(points, starting_b, starting_w, lr, num_iterations):
    ...
    w = starting_w
    # 根据梯度下降算法更新多次
    for step in range(num_iterations):                     # 不断训练学习
        # 计算梯度并更新一次
